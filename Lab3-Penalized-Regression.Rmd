...
If you have alpha = 1, you have:

$$
  \alpha = 1, \hat{\boldsymbol{\beta}}^{\text{el.net}} = \text{argmin}_{\boldsymbol{\beta}}\displaystyle({(\mathbf{Y}-\mathbf{X}\boldsymbol{\beta})^{T}(\mathbf{Y}-\mathbf{X}\boldsymbol{\beta})+{1 \lambda_1\|\boldsymbol{\beta}\|_{1}}+ {(1 - 1)\lambda_2\|\boldsymbol{\beta}\|_{2}^{2}}}\displaystyle)
 
 
$$
which then gives :
$$
 \alpha = 1, \hat{\boldsymbol{\beta}}^{\text{el.net}} = \text{argmin}_{\boldsymbol{\beta}}\displaystyle({(\mathbf{Y}-\mathbf{X}\boldsymbol{\beta})^{T}(\mathbf{Y}-\mathbf{X}\boldsymbol{\beta})+{ \lambda_1\|\boldsymbol{\beta}\|_{1}}}\displaystyle)
 
 
$$
which is the same as lasso
...
```{r}
plot(ridge_model, xvar = "lambda", xlab = "log(lambda)")
```
...
```{r}
lasso_cv = cv.glmnet(trainX,trainY,alpha=1,nfolds = 20,type.measure = "mse")
...
```{r}
final_model <- glmnet(X, Y, alpha = 1, lambda = 0.06887307)

lasso_preds =  predict(final_model, newx = testX, ncomp = optimal_ncomp)
...
```{r}
lasso_cv = cv.glmnet(trainX,trainY,alpha=0,nfolds = 20,type.measure = "mse")
...
```{r}
final_model <- glmnet(X, Y, alpha = 0, lambda =  9.318855)
